...
contract WalletFactoryScript is Script {
    ...
    IEntryPoint constant ENTRYPOINT = 
        ...
        function run() external {
            uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY"); // Fetch private key from env variables
            vm.startBroadcast(deployerPrivateKey); //Start broadcasting transactions

            WalletFactory walletFactory = new WalletFactory(ENTRYPOINT); // Initialize WalletFactory contract
